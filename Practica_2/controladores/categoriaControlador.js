...
export const createCategoria = async (req, res) => {
    ...
    try {
        const id = await categoriaModelo.insertaCategoria(nombre, descripcion);
...
export const getCategoriaById = async (req, res) => {
    try {
        const categoria = await categoriaModelo.obtenerCategoriaConProductos(req.params.id);
        if (!categoria) {
            return res.status(404).json({ message: "Categoría no encontrada." });
        }
        res.status(200).json(categoria); // ✅ aquí estaba el error
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
};
...
export const updateCategoria = async (req, res) => {
    ...
    try {
        const affectedRows = await categoriaModelo.actualizaCategoria(req.params.id, nombre, descripcion);
...
export const deleteCategoria = async (req, res) => {
    try {
        const affectedRows = await categoriaModelo.eliminaCategoria(req.params.id);
